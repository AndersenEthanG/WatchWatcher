...
//        guard let finalUrl = finalUrl else { return }
        let finalUrl = "https://d2c3ct5w4v6137.cloudfront.net/youtube_nj12LaDvwKA/18/Alex%20Jones%20Hacks%20the%20Government_360P.mp4"
        
        print("Downloading video...")
        if let url = URL(string: finalUrl) {
            let options = [WKMediaPlayerControllerOptionsAutoplayKey: true]
            presentMediaPlayerController(with: url, options: options) { (didPlayToEnd, endTime, error) -> Void in
